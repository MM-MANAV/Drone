...
```
C:\Users\Stavan\Downloads\Drone-main\Drone-main\
│
├── src\app\main.py          ← FastAPI backend (COM port + API)
...
```cmd
cd C:\Users\Stavan\Downloads\Drone-main\Drone-main
```
...
```cmd
cd C:\Users\Stavan\Downloads\Drone-main\Drone-main\frontend
npm run dev
```
...
The chatbot inside the dashboard will use this local LLM to answer questions based on documents placed in `data\documents\`.
...
- If you changed the backend port, update `frontend\next.config.mjs`
